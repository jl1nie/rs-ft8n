...
各 WAV には 15 局の強信号 crowd と、微弱なターゲット局 **CQ 3Y0Z JD34** が含まれる。

| WAV | WSJT-X | rs-ft8n WASM (subtract) | 比較ポイント |
|-----|--------|------------------------|-------------|
| `sim_busy_band.wav` | 7 局 | **16 局（3Y0Z 含む）** | OSD 深度の差 |
| `sim_stress_fullband.wav` | 10 局（3Y0Z なし） | **15 局（3Y0Z なし）** | ADC 飽和で 3Y0Z 消失（双方） |
| **`sim_stress_bpf_edge_clean.wav`** | **デコード不能** | **CQ 3Y0Z JD34 (197 ms)** | **BPF + EQ + AP の効果** ※ |
