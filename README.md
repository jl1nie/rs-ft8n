...
**WSJT-X との性能比較について:** WSJT-X はデコーダを別プロセス (jt9) として GUI と分離し、1 受信期間中に 3 回の早期デコードを段階的に起動する（symbol 41/47/50 時点）。ただし各デコード呼び出し内の**候補ループはシリアル**（OpenMP 不使用）であり、マルチコア CPU を活かせていない。rs-ft8n は **Rayon で候補を並列デコード** することで最大 7.7 倍の高速化を実現している。Fortran デコーダコア自体は数値計算として高速であり、シングルスレッドでの性能差は言語差ではなく並列化戦略の違いによる。
