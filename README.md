...
> ※ rs-ft8n はターゲット局コール (3Y0Z) を事前指定した AP デコードと、BPF 肩歪みを補正する適応型等���器 (EQ) を併用している。WSJT-X でも DX Call に 3Y0Z を設定して AP を有効化したがデコードには至らなかっ���。EQ による BPF 肩補正が効いていると考えられる。AP なし（EQ のみ）でも -18 dB BPF edge で 30% のデコード率がある（[詳細](#bpf-edge-snr-sweep--bpf--eq--ap-の累積効果)）。
